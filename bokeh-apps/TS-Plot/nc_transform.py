...
def get_tsp_data_dict(tsp):
    ...
    tsp_dict = {key.round('1s').strftime("%Y-%m-%d %H:%M:%S"): pd.DataFrame(np.array([data[i].values for i in data.columns]).T, 
        index=data.index.get_level_values(vertical_level).values, 
        columns=data.columns).rename_axis(vertical_level) for key,data in tsp.groupby(level=time_level)}
    return tsp_dict
...
def get_vp_data_new_old(profile):
    ...
    if len(profile.index.names) == 2:
        ...
        profile_dict = {j:pd.DataFrame.from_dict({str(v): df.loc[[df.index.get_level_values(time_level)[i]]].reset_index(level=time_level, drop=True)[
                j].values for i, v in enumerate(df.index.unique(level=time_level))}) for j in plottable_variables}
        ...
        for i in plottable_variables:
            ...
            values = [str(j).rsplit('.')[0] for j in pd.DatetimeIndex(profile_dict[i].columns).round('1s').values]
